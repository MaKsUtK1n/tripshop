...
class CryptoPay:
    ...
    def create_invoice(self, amount: float) -> dict:
        res = post(self.url + "createInvoice", json={'amount': amount, "currency_type": 'fiat', 'fiat': 'usd'}, headers=self.headers).json()
    ...
    def get_invoice(self, invoice_id) -> dict:
        res = post(self.url + "getInvoices", json={'fiat': 'usd', 'invoice_ids': invoice_id}, headers=self.headers).json()
        if 'error' in res and res['error']['name'] == "UNAUTHORIZED":
            raise errors.UNAUTHORIZED("ТОКЕН НЕ ПОДХОДИТ")
...
if __name__ == "__main__":
    send = CryptoPay("176102:AA5j7RnscqMCrTgqcuy3cmjP2EKhvS1tptT")
